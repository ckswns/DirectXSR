...
BillBordParticle::BillBordParticle(LPDIRECT3DDEVICE9 pDev) noexcept
	...
	_iMaxParticles(50), _fEmitRate(5), _fEmitTime(0),
	_bLoop(true), _fSpeed(1.f), _fTime(0), _fDuration(5.f)
{}
...
{
	if (fElapsedTime >= (1.f / _fEmitRate))
	{
		AddParticle();
	}
	else
	...
	for (iter = _pParticles.begin(); iter != _pParticles.end(); iter++)
	{
		if ((*iter)->_bIsAlive)
		{
			...
			if ((*iter)->_fAge > (*iter)->_fLifeTime)
			{
				if (_bLoop) 
				{
					if(_pParticles.size() < _iMaxParticles)
						ResetParticle((*iter));
					else
