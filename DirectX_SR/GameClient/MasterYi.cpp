...
void MasterYi::Awake(void) noexcept
{
	_state = Actor::State::IDLE;
	memcpy(&_data, &GAMEDATAMANAGER->GetActorData("MasterYi"), sizeof(Actor::Data));
}
...
void MasterYi::Start(void) noexcept
{
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 14; j++)
		{
			...
			frameTex.push_back(ASSETMANAGER->GetTextureData("Asset\\Actor\\Monster\\MasterYi\\Idle\\" + std::to_string(index) + ".png"));
		}
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 14; j++)
		{
			...
			frameTime.push_back(0.1f);
			frameTex.push_back(ASSETMANAGER->GetTextureData("Asset\\Actor\\Monster\\MasterYi\\Walk\\" + std::to_string(index) + ".png"));
		}
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 14; j++)
		{
			int index = i * 14 + j + 1;
			frameTime.push_back(0.1f);
			frameTex.push_back(ASSETMANAGER->GetTextureData("Asset\\Actor\\Monster\\MasterYi\\Attack\\" + std::to_string(index) + ".png"));
		}

		Animation* ani = new Animation(frameTime, frameTex, false);
		ani->SetMaterial(_spriteRenderer->GetMaterialPTR());
		_animator->InsertAnimation("Attack_" + std::to_string(i), ani);
		ani->AddEvent(Animation::EventData(4, "attack", this->gameObject));
	...
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 11; j++)
		{
			int index = i * 11 + j + 1;
			frameTime.push_back(0.1f);
			frameTex.push_back(ASSETMANAGER->GetTextureData("Asset\\Actor\\Monster\\MasterYi\\Death\\" + std::to_string(index) + ".png"));
		}
...
void MasterYi::FixedUpdate(float fElapsedTime) noexcept
{
...
void MasterYi::Update(float fElapsedTime) noexcept
{
	switch (_state)
	{
	case Actor::State::IDLE:
...
void MasterYi::LateUpdate(float fElapsedTime) noexcept
{
...
void MasterYi::OnDestroy(void) noexcept
{
...
void MasterYi::OnCollisionEnter(Collider* mine, Collider* other) noexcept
{
}

void MasterYi::OnCollisionStay(Collider* mine, Collider* other) noexcept
{
}

void MasterYi::OnCollisionExit(Collider* mine, Collider* other) noexcept
{
}

void MasterYi::GetHit(int damage) noexcept
{
...
void MasterYi::OnAnimationEvent(std::string str) noexcept
{
	D3DXVECTOR3 target = _player->GetTransform()->GetWorldPosition();
	D3DXVECTOR3 mine = transform->GetWorldPosition();
