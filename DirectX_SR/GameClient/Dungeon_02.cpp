...
bool Dungeon_02::Init(void) noexcept
{
	NaviMesh* _pNaviMesh;

	Terrain* terrain = new Terrain(INIMANAGER->LoadDataInteger("Asset\\Scene\\Dungeon_02\\Terrain", "Terrain", "MapWidth") + 1,
		INIMANAGER->LoadDataInteger("Asset\\Scene\\Dungeon_02\\Terrain", "Terrain", "MapHeight") + 1, 1);
	{
		...
		{
			char str[256];
			sprintf_s(str, 256, "Asset\\Scene\\Dungeon_02\\AlphaMap\\AlphaMap%d.png", i);
		...
		int cubeCnt = INIMANAGER->LoadDataInteger("Asset\\Scene\\Dungeon_02\\PickingCube", "Start", "CubeCnt");

		for (int i = 0; i < cubeCnt; i++)
		{
			obj = GameObject::Instantiate();
			std::string sectionName = "PickingCube" + std::to_string(i);
			std::string filePath = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "filePath");
			filePath = filePath.substr(filePath.find('\\'));
			filePath = "Asset\\Terrain\\Box" + filePath;

			std::string sx = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "scaleX");
			std::string sy = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "scaleY");
			std::string sz = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "scaleZ");

			std::string px = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "worldposX");
			std::string py = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "worldposY");
			std::string pz = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\PickingCube", sectionName.c_str(), "worldposZ");
			...
			if (stof(sz) > stof(sx))
			{
				if (INIMANAGER->LoadDataInteger("Asset\\Scene\\Dungeon_02\\Terrain", "Terrain", "MapWidth") / 2 < stof(px))
	...
	std::string name = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\Object", "SectionNames", "Names");
	...
	for (int i = 0; i < names.size(); i++)
	{
		if (names[i].find("Tree") != std::string::npos)
		{
			...
			std::string px = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\Object", names[i].c_str(), "worldX");
			std::string py = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\Object", names[i].c_str(), "worldY");
			std::string pz = INIMANAGER->LoadDataString("Asset\\Scene\\Dungeon_02\\Object", names[i].c_str(), "worldZ");
	...
	for (int i = 0; i < 30; i++)
	{
		...
		D3DXVECTOR3 pos = { 20, 0, 20 };

		pos.x += Random::GetValue(3, 20);
		pos.x -= Random::GetValue(3, 20);
		pos.y = 0.7f;
...
void Dungeon_02::FixedUpdate(float fElapsedTime) noexcept
{

}

void Dungeon_02::Update(float fElapsedTime) noexcept
{

}

void Dungeon_02::LateUpdate(float fElapsedTime) noexcept
{
